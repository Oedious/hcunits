...
SET_MAP = {
  ...
  "tt": {
    "name": "Teen Titans",
    "ranges": [
      # TODO: Add support for resources and R101 Robin Uniform
      ("tt001", "ttG001r"),
      ("ttS101", "ttT006r"),
    ],
  },
  ...
  def parse_unit_page(self, unit_page):
    ...
    if (figure_rank == "" or
      ...
      if is_construct:
        ...
        if has_parent and (self.name.startswith("Spell:") or self.name.startswith("Trap:")):
          print("Skipping spell or trap for %s" % self.unit_id)
          return False
      ...
      for tag in tag_list:
        ...
        for part in parts:
          ...
          if part == "Light Object" or part == "Light: Continual":
            ...
            if match_obj:
              if len(match_obj.group(1)) > 0:
                self.relic_roll_min = int(match_obj.group(1))
              else:
                self.relic_roll_min = 6
            else:
    ...
    if (self.type == "character" or
      ...
      if len(tag_list) > 0 and tag_list[0].parent.parent.parent.parent.tbody:
        ...
        for tr_tag in sp_tag.parent.parent.parent.parent.tbody.find_all('tr', recursive=False):
          ...
          # Skip the special power that describes rules.
          if (self.type == "character" and sp_name and
              (sp_name.upper().startswith("SPECIAL TERRAIN TRAIT") or
               sp_name == "SPLIT" or
               sp_name == "MERGE" or
               sp_name == "TINY SIZE")):
            continue
          ...
          if sp_type_str == "special" or sp_type_str == "prereq":
            ...
            if self.set_id == "wol":
              sp_type = "construct"
            elif self.set_id == "dp" or self.set_id == "ffdp":
            ...
            if self.set_id.startswith("ygo"):
              sp_type = "spell"
          elif sp_type_str == "purple-circle":
            if self.set_id.startswith("ygo"):
              sp_type = "trap"
      ...
      for col in dials[0].find_all("table", class_="power"):
        ...
        if tags[0].td.has_attr('title'):
          ...
          for i in range(4):
            ...
            # Check for special-special power types like traps and spells.
            if not power:
              attrs = tag.td['class']
              if len(attrs) > 0:
                if attrs[0] == "blue-circle" and value == "S":
                  power = "spell"
                  value = ""
                if attrs[0] == "purple-circle" and value == "T":
                  power = "trap"
                  value = ""
    ...
    if self.type == "character" or self.type == "bystander":
      ...
      for click in self.dial:
        ...
        for field in ["speed", "attack", "defense", "damage"]:
          value = click.get(field + "_value", None)
